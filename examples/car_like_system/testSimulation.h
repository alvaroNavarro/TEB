...
class Test 
 ...
 public:
   ...
   Test(teb::TebController<p,q>* robController, MobileRobotCarLikeSystem<p,q>& robot, teb::Simulator<p,q>* sim, teb::TebPlotter* plotter) : _robController(robController), _robot(&robot), _sim(sim), _plotter(plotter) 
   {      
     _pws = getpwuid(geteuid());
     _folder_simulation  = "/Desktop/Simulations/";    
   }     
   ...
   bool prepareDynamicObstacle(const Eigen::Ref<const Eigen::Vector2d>& obs_pos_ini, const Eigen::Ref<const Eigen::Vector2d>& obs_pos_final, unsigned int op_vel)
   {
      _robController->addObstacle(new teb::Obstacle(obs_pos_ini, 2));              //! In this moment the radius will be fixed...
      _robController->getObstacle()->setObstaclePosition(obs_pos_final, "final_fixed");
      
      if(op_vel == 1)
      {
	double vel;
	PRINT_INFO("Please enter obstacle velocity: ");
	INPUT_STREAM(vel, -1);
	_robController->getObstacle()->setObstacleVelocity(vel);
	
	if(_robController->getObstacle()->setObstacleDeltaX(_robController->dt().dt()) == false)
	{
	  PRINT_INFO("The velocity is too large..");
	  return false;
	}
      }
                        
      _robController->getObstacle()->initializeObstaclePath();
      return true;
   }
   ...
   void menu();
   void run();  
   void runSimulationDynamicObstacle(const Eigen::Ref<const Eigen::Vector2d>& obs_pos_ini, const Eigen::Ref<const Eigen::Vector2d>& obs_pos_final, unsigned int op_vel);
   ...
   {           
	 ...
	 _final_state   = getPredefinedState(1,"final");
	  ...
	  getArrayStates(); 
       ...
       else if(_type_scenario == TypeScenario::DYNAMIC_OBSTACLE)
       {
	  //! The initial and final state are predefined... 
	 teb::InitialFinalStates states_values;	 
	 states_values.getInitialFinalState(_x0, _xf, 8);     //! Based on the initial and final state saved from the beginning, we just
							      // take one of them...	 	 
	 _initial_state = Eigen::Map<StateVector>(_x0, _robot->DimStates);
	 _final_state   = Eigen::Map<StateVector>(_xf, _robot->DimStates);
   ...
   void saveDynamicObstaclePath(const struct teb::SimResults::TimeSeries& ts);
 ...
 protected:      
   ...
   std::string _fileNameDynamicObs; 
